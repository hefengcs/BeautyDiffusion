        ...
        for self.epoch in range(1, self.num_epochs + 1):
            ...
            with tqdm(data_loader, desc="training") as pbar:
                for step, (source, reference) in enumerate(pbar):
                    ...
                    # source = torch.load('/home/hefengcs/makeup/demo/EleGANt/assets/pt/source.pt')
                    # reference = torch.load('/home/hefengcs/makeup/demo/EleGANt/assets/pt/reference.pt')
                    #
                    # source = [t.unsqueeze(0) for t in source]
                    # reference = [t.unsqueeze(0) for t in reference]
                    ...
                    pbar.set_description("Epoch: %d, Step: %d, Loss_G: %0.4f, Loss_A: %0.4f, Loss_B: %0.4f" % \
                                         (self.epoch, step + 1, np.mean(losses_G), np.mean(losses_D_A),
                                          np.mean(losses_D_B)))
