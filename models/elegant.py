...
class Generator(nn.ModuleDict):
    ...
    def __init__(self, conv_dim=64, image_size=256, num_layer_e=2, num_layer_d=1, window_size=16, use_ff=False,
        ...
        # -------------------------- Encoder --------------------------
        #
        layers = nn.Conv2d(3, conv_dim, kernel_size=7, stride=1, padding=3, bias=False) #2维卷积，输出64个特征
        self.add_module('in_conv', layers)

        # Down-Sampling & Bottleneck
        curr_dim = conv_dim; feature_size = image_size
        for i in range(2):
            layers = Downsample(curr_dim, curr_dim * 2, affine=True) #下采样，尺寸减半，通道数翻倍
    ...
    def forward(self, c, s, mask_c, mask_s, diff_c, diff_s, lms_c, lms_s):
        ...
        lms: (b, K, 2)
        """
        transfer_input_c = self.get_transfer_input(c, mask_c, diff_c, lms_c) #source image  #两个input用于进行特征提取
        transfer_input_s = self.get_transfer_input(s, mask_s, diff_s, lms_s, True) #reference image
